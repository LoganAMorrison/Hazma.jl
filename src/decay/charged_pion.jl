...
"""
    dndeᵧ_π_lνγ_πrf(eγ::Real, ml::Real)
...
"""
function dndeᵧ_π_lνγ_πrf(eγ::Real, ml::Real)
...
"""
    dndeᵧ_π_decay(eγ::Real, eπ::Real[;mode::String=total])
...
"""
function dndeᵧ_π_decay(eγ::Real, eπ::Real; mode::String = "total")
    eπ < mπ && return zero(typeof(eγ))

    dndeᵧ_π_eνγ_πrf(engγ) = br_π_eν * dndeᵧ_π_lνγ_πrf(engγ, me)
    dndeᵧ_π_μνγ_πrf(engγ) = br_π_μν * dndeᵧ_π_lνγ_πrf(engγ, mμ)
    dndeᵧ_π_μν_πrf(engγ) =
        br_π_μν * dndeᵧ_μ_decay(engγ, (mπ^2 + mμ^2) / (2 * mπ))
    spectrum_rf(engγ) =
        (dndeᵧ_π_eνγ_πrf(engγ) + dndeᵧ_π_μνγ_πrf(engγ) + dndeᵧ_π_μν_πrf(engγ))

    if mode == "total"
        return boost_spectrum(spectrum_rf, eπ, mπ, eγ, 0.0)
    elseif mode == "μν"
        return boost_spectrum(dndeᵧ_π_μν_πrf, eπ, mπ, eγ, 0.0)
    elseif mode == "μνγ"
        return boost_spectrum(dndeᵧ_π_μνγ_πrf, eπ, mπ, eγ, 0.0)
    elseif mode == "eνγ"
        return boost_spectrum(dndeᵧ_π_eνγ_πrf, eπ, mπ, eγ, 0.0)
    else
